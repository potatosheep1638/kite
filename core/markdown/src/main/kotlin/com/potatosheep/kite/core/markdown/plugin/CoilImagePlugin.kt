package com.potatosheep.kite.core.markdown.plugin
        ...
        private inner class AsyncDrawableTarget(
            ...
            override fun onSuccess(result: coil3.Image) {
                ...
                if (cache.remove(drawable) != null
                    ...
                    if (drawable.isAttached) {
                        val drawableImage = result.asDrawable(Resources.getSystem()).mutate()

                        DrawableUtils.applyIntrinsicBoundsIfEmpty(drawableImage)
                        drawable.result = drawableImage
                    }
                }
            }
            ...
            override fun onStart(placeholder: coil3.Image?) {
                if (placeholder != null && drawable.isAttached) {
                    val drawableImage = placeholder.asDrawable(Resources.getSystem()).mutate()
            ...
            override fun onError(error: coil3.Image?) {
                if (cache.remove(drawable) != null) {
                    if (error != null && drawable.isAttached) {
                        val drawableImage = error.asDrawable(Resources.getSystem()).mutate()
