pluginManagement {
    includeBuild("build-logic")
    repositories {
        google {
            content {
                includeGroupByRegex("com\\.android.*")
                includeGroupByRegex("com\\.google.*")
                includeGroupByRegex("androidx.*")
            }
        }
        mavenCentral()
        gradlePluginPortal()
    }
}
dependencyResolutionManagement {
    repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)
    repositories {
        google()
        mavenCentral()
    }
}

rootProject.name = "kite"

enableFeaturePreview("TYPESAFE_PROJECT_ACCESSORS")
include(":app")
include(":core:network")
include(":core:model")
include(":core:data")
include(":feature")
include(":feature:feed:impl")
include(":feature:feed:api")
include(":core:ui")
include(":core:designsystem")
include(":feature:subscription")
include(":feature:home:impl")
include(":feature:home:api")
include(":feature:post:impl")
include(":feature:post:api")
include(":feature:image:impl")
include(":feature:image:api")
include(":feature:subreddit:impl")
include(":feature:subreddit:api")
include(":core:common")
include(":feature:subredditwiki")
include(":feature:user:impl")
include(":feature:user:api")
include(":feature:searchresult:impl")
include(":feature:searchresult:api")
include(":core:markdown")
include(":feature:video:impl")
include(":feature:video:api")
include(":core:domain")
include(":feature:settings:impl")
include(":feature:settings:api")
include(":core:datastore")
include(":core:datastore-proto")
include(":core:database")
include(":feature:bookmark:impl")
include(":feature:bookmark:api")
include(":feature:exception")
include(":feature:onboarding:impl")
include(":feature:onboarding:api")
include(":feature:about:impl")
include(":feature:about:api")
include(":core:media")
include(":core:notification")
include(":core:translation")
include(":core:navigation")
